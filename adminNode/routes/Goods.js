...
router.post('/delete', function(req, res, next) {
    ...
    if (req.body.id) {
        sql.find(goodsinfo,{id: req.body.id}).then( (data) =>{
            if (data.length === 1 ) {
                sql.update(goodsinfo,{id: req.body.id}, {state: -1}).then((err,data) =>{
                        res.send({
                            "code": "1",
                            "msg": "删除成功"
                        })
...
router.get('/find', function(req, res, next) {
    ...
    if(whereObj.range_gt && whereObj.range_lt) {
        whereObj.normal_price = {$gt: whereObj.range_gt, $lt: whereObj.range_lt}
    }else if (whereObj.range_gt) {
        whereObj.normal_price ={ $gt: range_gt}
        // console.log(whereObj)
        // console.log(whereObj.normal_price)
    }else if ( whereObj.range_lt) {
        whereObj.normal_price = { $lt: range_lt}
    }
    if (whereObj.count) {
